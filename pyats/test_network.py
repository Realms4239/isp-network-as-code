...
class BGPPeeringCheck(aetest.Testcase):
    """ Assert BGP sessions are Established on FRR transit node """
    @aetest.test
    def verify_bgp_established(self, testbed):
        ...
        # Ensure neighbors 10.1.13.1 and 10.1.23.1 are established
...
class EndToEndReachabilityCheck(aetest.Testcase):
    ...
    def verify_ping_transit(self, testbed):
        ...
        ping_res = srl01.execute("ping 10.0.0.3 -c 3")
        log.info(f"Ping output: {ping_res}")
        assert "0% packet loss" in ping_res or "3 received" in ping_res, "ICMP packet loss detected to 10.0.0.3"
